...
async def empty_username(exception: ErrorEvent, texts: Texts, user_lang: str) -> Any:
    assert exception.update.message is not None
    await exception.update.message.answer(texts["uname_error"][user_lang])
    
@router.errors()
async def any_exception(exception: ErrorEvent, texts: Texts, user_lang: str) -> Any:
    assert exception.update.message is not None
